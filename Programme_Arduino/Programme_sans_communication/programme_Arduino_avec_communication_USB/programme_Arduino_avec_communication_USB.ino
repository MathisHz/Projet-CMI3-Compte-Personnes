...
bool passage_fini = false;
unsigned long dernier_temps = 0;
const int delai_reset = 500; // 500 ms pour éviter les doubles comptages

void setup() {
    Wire.begin();
    Serial.begin(9600); // Communication USB série
    ...
    Serial.println(0); // Valeur initiale
}

void loop() {
    unsigned long temps_actuel = millis();

    // Lire les distances une seule fois
    int dist1 = hc1.dist();
    int dist2 = hc2.dist();
    ...
    // Réinitialisation des flags après le passage
    if (passage_fini && (temps_actuel - dernier_temps > delai_reset)) {
        passage_fini = false;
        hc1_detect = false;
        hc2_detect = false;
